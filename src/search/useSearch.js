...
export default function useSearch(originalSearchTerm) {
  ...
  searchInventory(originalSearchTerm); // initial search

  onMounted(() => console.log('Mounted: useSearch'));

  return { searchResults: results, search: searchInventory }; // search is passed to enable reusability in the other components
}